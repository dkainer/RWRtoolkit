...
make_heterogeneous_multiplex <- function(nw.groups, delta, lambda, out, verbose) {
    ...
    cat("constructing multiplex heterogeneous network from 2 groups of layers plus bipartite links:\n",
            nw.groups[[1]]$nwname,"\n",
            nw.groups[[2]]$nwname,"\n",
            nw.groups[[3]]$nwname,"\n")
    ...
    bipartite_links <- bipartite_links %>% dplyr::filter(from %in% nw.mpo1$Pool_of_Nodes) %>% dplyr::filter(to %in% nw.mpo2$Pool_of_Nodes)
