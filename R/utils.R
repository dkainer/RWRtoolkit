...
get_or_set_tau = function(nw.mpo, optTau) {
    ...
    if (sum(tau) != nw.mpo$Number_of_Layers || length(tau)!=nw.mpo$Number_of_Layers) {
        ...
        warning(sprintf("WARNING:: Your comma-delimited tau parameter values (%s) has the incorrect length or does not add up to the number of network layers (%s).  Automatically re-set tau to: %s", 
...
calc_ROCPRC <- function(df, scorescol=NULL, labelscol=NULL, totP = NULL) {
    ...
    if(is.null(totP))
        totP    <- sum(get(labelscol, df)==1) # number of positives in gold set
