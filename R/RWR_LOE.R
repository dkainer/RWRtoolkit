####################################################################################################
# Perform RWR from one geneset and returns either (1) ranking of the rest of the genes in the 
...
calc_metrics_loe <- function(res, seed_geneset, query_geneset) {
    ...
    # then manually append them to the top of RWR results with the best possible rank.
    # This is because RWR_LOE does not provide ranks to genes in the seed geneset,
    # but in this scenario we need them to have a top rank because technically
    # they represent exact overlap between geneset1 and geneset2


    # old method
    # dupes <- which(query_geneset$gene %in% seed_geneset$gene)
    dupes <- dplyr::slice(query_geneset, which(query_geneset$gene %in% seed_geneset$gene))

    if(nrow(dupes) > 0) {
        warning(sprintf("WARNING: %i genes in the query geneset are also found in the seed geneset. They will be removed from the query geneset before running eval", length(dupes)), appendLF = T)
         res <- rbind( 
                    data.frame(
    ...
    res <- calc_ROCPRC(df=res, scorescol = "rank", labelscol = "InQueryGeneset")
    
    filtervalue <- dplyr::filter(res, rank == nrow(!!query_geneset))
    outputvalue <- filtervalue  %>% dplyr::pull(PREC)

    # 1. precision @ size(query_geneset) (aka R-PREC)
    output <- data.frame(value=ifelse(length(outputvalue) > 0, outputvalue, NA), 
                         measure = "P@SizeQueryGeneSet")

    # 2. Avg PRC (uses Average Precision, not interpolated precision)
    output <- rbind(output, dplyr::filter(res, TP==1) %>% dplyr::summarise(value = sum(PREC)/sum(InQueryGeneset), measure="AvgPrec") )

    # 3. AUPRC
    output <- rbind(output, dplyr::summarise(res, value = area_under_curve(REC, PREC, method="trapezoid", ties="max"), measure="AUPRC"))               

    # 4. AUROC
    output <- rbind(output, dplyr::summarise(res, value = sum( REC )/dplyr::n(), measure="AUROC"))

    # 5. NDCG
    output <- rbind(output, dplyr::filter(res, rank == nrow(!!query_geneset)) %>% 
                        dplyr::summarise(value = ndcg, measure="NDCG@SizeQueryGeneSet"))

    output <- rbind(output, dplyr::summarise(res, value = area_under_curve(REC, ndcg, method="trapezoid", ties="max"), measure="AUNDCG"))
    
    return(list(summary = output, results = res)) 
}

#' @keywords internal
view_top_network_loe <- function(results, nw.mpo, ntop=199, seed_geneset, query_geneset=NULL, cyto, modname) {
    if (cyto) {
        topresults <- RandomWalkRestartMH::create.multiplexNetwork.topResults(results, nw.mpo, k=ntop)
        igraph::V(topresults)$label.cex <- 0.6
        if (!is.null(query_geneset)) {
            igraph::V(topresults)$type <- dplyr::case_when(igraph::V(topresults)$name %in% seed_geneset$gene ~ "InSet1",
        ...
        igraph::V(topresults)$Rank <- NA
        tmp <- match(igraph::V(topresults)$name, results$RWRM_Results$NodeNames)
        igraph::V(topresults)$Rank <- results$RWRM_Results$rank[tmp]
        
        #pallete1 <- RColorBrewer::brewer.pal(length(unique(E(topresults)$type)), name = "Set3")
        #pallete2 <- RColorBrewer::brewer.pal(length(unique(V(topresults)$type)), name = "Blues")

        #E(topresults)$color <- pallete1[as.factor(E(topresults)$type)]
        #V(topresults)$color <- pallete2[as.factor(V(topresults)$type)]

        RCy3::cytoscapePing()
        RCy3::createNetworkFromIgraph(topresults, title=paste0("seeds_to_top",ntop), collection=modname)
        RCy3::layoutNetwork('kamada-kawai')
        #RCy3::setEdgeColorMapping(table.column = "type", colors = pallete1, table.column.values = unique(E(topresults)$type), mapping.type="d")
        #RCy3::setNodeColorMapping(table.column = "type",
        #                          table.column.values = levels(factor(V(topresults)$type)),
        #                          colors = pallete2,
        #                          mapping.type = "d")
        #RCy3::setNodeSizeMapping(table.column = "Rank", mapping.type = "c")
...
#' @keywords internal
save_plots_loe <- function(metrics, seed_geneset, query_geneset, outdir, modname) {
    # this uses custom scoring and avoids the use of Precrec library
    if (is.null(outdir)) {
        message('You must provide a path to --outdir to save the metrics and plots.')
        return(1)
    }
    ...
    # REC = metrics$results$REC
    # PREC = metrics$results$PREC
    # FPR = metrics$results$FPR
    # rank = metrics$results$rank
    # ndcg = metrics$results$ndcg
    # plot PRC
    p1 <- ggplot2::ggplot(metrics$results) + 
        ggplot2::geom_path(ggplot2::aes(x=REC, y=PREC), col="darkorange", alpha=0.5) +
        ggplot2::ylim(c(0,1)) + 
        ggplot2::xlab("Recall") +
    ...
    p2 <- ggplot2::ggplot(metrics$results) + 
        ggplot2::geom_line(ggplot2::aes(x=FPR, y=REC), col="darkorange", alpha=1) +
        ggplot2::geom_abline(ggplot2::aes(intercept=0, slope=1), linetype="dashed", col="darkgrey") +
        ggplot2::xlab("FPR") +
        ggplot2::ylab("TPR")
    
    # plot NDCG for top N ranks
    p3 <- ggplot2::ggplot(metrics$results %>% dplyr::filter(rank %in% c(5,10,20,30,40,50,100,200,300,400,500))) + 
...
#' @param seed_geneset The path to the gene set file. It must have the following first two columns with no headers tab-delimited: {<}setid{>} {<}gene{>} {<}weight{>}
#' @param query_geneset The path to the optional second gene set file. It must have the following first two columns with no headers tab-delimited: {<}setid{>} {<}gene{>} {<}weight{>}
#' @param restart Sets the restart parameter [0,1). Higher value means the walker will jump back to a seed node more often. Defaulted to 0.7
...
#' @param numranked Proprtion of ranked genes to return \[0,1\].  e.g. 0.1 will will return the top 10%. Default 1.0
...
#' @param cyto Specify a number N > 0 if you wish to see a network of the seeds and top N ranked genes (cytoscape must already be running)
#' @param verbose Verbose mode (Default False)
#' @return Returns a list of (1) a dataframe of results (scores for genes) and (2) the seed genes. 
#' @examples
...
#' loe_gene_seed_list <- RWR_LOE(data=multiplex_object_filepath,
...
#' loe_gene_seed_list2 <- RWR_LOE(data=multiplex_object_filepath,
#'         seed_geneset=geneset1_filepath,
...
#'
#' @export
RWR_LOE <- function(data=NULL, seed_geneset=NULL, query_geneset=NULL, restart=0.7, tau="1.0", outdir="./", numranked=1.0, eval=FALSE, modname="default", cyto=0, verbose=FALSE)
{
    # Check parameters and print key parameters on verbose mode
    if (is.null(data)) stop("ERROR: Mandatory arguement data is missing.")
    if (is.null(seed_geneset)) stop("ERROR: Mandatory arguement seed_geneset is missing.")
    if (!file.exists(data)) stop("ERROR: Rdata input file does not exist: ", data)
    if (verbose) {
        cat(paste("\nRdata file:     ",data,"\nSeed geneset: ",seed_geneset,"\nQuery geneset: ",query_geneset,"\noutput: ",outdir,"\nproportion of ranked nodes to be saved: ",numranked,"\n\n"))
    }

    # Load the multiplex network and adjacency matrices (i.e. nw.mpo, nw.adj, nw.adjnorm)
    load(data)
    ...
    tau = get_or_set_tau(nw.mpo, optTau=tau)

    # Load the gene sets, or set to NULL (utility function)
    seed_geneset  <- load_geneset(seed_geneset, nw.mpo, verbose)$geneset
    query_geneset <- load_geneset(query_geneset, nw.mpo, verbose)$geneset # will return NULL if not set by user
    
    # Core of method
    message('\nBeginning RWR_LOE ...')
    
    # Set network_names to be the concatenation of individual network names
    network_names    <-  paste(names(nw.mpo)[1:nw.mpo$Number_of_Layers], collapse = "_")
    
    # Run RWR on multiplex network starting at seed genes defined by seed_geneset
    results     <- RandomWalkRestartMH::Random.Walk.Restart.Multiplex( x = nw.adjnorm, MultiplexObject = nw.mpo, Seeds = seed_geneset$gene, r = restart )
    
    # Add a rank column to the results (min_rank gives the minimum rank on a tie, skips next ranking)
    # Any gene that scored 0, give it worst possible rank
    results$RWRM_Results <- results$RWRM_Results %>% 
        dplyr::mutate(rank = dplyr::min_rank(-Score)) %>% 
        dplyr::mutate(rank = dplyr::if_else(Score==0, true = nw.mpo$Number_of_Nodes_Multiplex, false = rank))  
    # Add in additional columns to results
    results$RWRM_Results <- results$RWRM_Results %>%
        dplyr::mutate(num_in_network = nrow(seed_geneset),
    ...
    if (!is.null(query_geneset)) {
        message('A query geneset was provided.  Results will be filtered for only genes from the query geneset.')
        results$RWRM_Results  <- results$RWRM_Results %>% 
            tibble::add_column(query_geneset = query_geneset$setid[1]) %>% 
            tibble::add_column(InQueryGeneset = as.numeric(results$RWRM_Results$NodeNames %in% query_geneset$gene))  
    } else {
    ...
    write_table(results$RWRM_Results %>% dplyr::slice_head(prop=numranked), out_path, verbose=TRUE)
    ...
    {
        warning(sprintf("Eval mode is turned ON but there is no query geneset, evaluation will not occur."))
    }

    # If cytoscape mode is turned on, open an interactive cytoscape session.
    if (cyto > 0) {
        message("opening top n network in cytoscape")
        view_top_network_loe(results, nw.mpo, ntop=cyto, seed_geneset, query_geneset, cyto, modname)
    }
