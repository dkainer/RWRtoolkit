...
### Network Overlap
With `RWR_netstats` we can compare networks by calculating jaccard similarity coefficient or overlap scores with respect to the shared edges within the networks.

When calculating the jaccard similarity coefficient between two networks A and B, we create sets of their edges: 
...
#### Comparing All Layers of a Multiplex
You'll likely wish to compare each of the layers within a multiplex network as well, this can be done using `overlap_score` flag to obtain the pairwise overlap scores OR the `overlap_sim_multiplex` to obtain the pairwise Jaccard similarity coefficients: 
...
netstats$overlap_sim_multiplex_jaccard

print("The pairwise overlap scores are:")
netstats$mpo_overlap_score
```

#### Comparing All Layers of a Multiplex against a Reference Network
Just as you can compare a single layer to a reference network, it's also possible to compare all layers against a reference network: 
...
pal <- brewer.pal(n_colors, "Spectral")
